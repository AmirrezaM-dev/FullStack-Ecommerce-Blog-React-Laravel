...
; (function ($) {
	...
	jQuery.extend(true, _R, {
		...
		updateMarkup: function (layer, o) {
			...
			if (d.start && !d.frames_added && d.frames === undefined) {
			...
			if (!d.frames_added) {
				...
				if (d.frames) {
					for (var i = 0; i < d.frames.length; i++) {
						...
						if (d.frames[i].sfx_effect && d.frames[i].sfx_effect.indexOf('block') >= 0) {
							if (i === 0) {
								d.frames[i].from = "o:0";
								d.frames[i].to = "o:1";
							} else {
						...
						if (d.frames[i].split && d.frames[i].split.match(/chars|words|lines/g))
							d.splittext = true;
					}
	...
	prepareSingleCaption: function (obj) {
					...
					if (_nc.find('img').length > 0) {
						...
						if (_._ingroup && _._nctype !== "row") {
							if (ww && !jQuery.isNumeric(ww) && jQuery.type(ww) === "string" && ww.indexOf("%") > 0)
								punchgs.TweenLite.set([_._lw, _._pw, _._mw], { minWidth: ww });


							if (hh && !jQuery.isNumeric(hh) && jQuery.type(hh) === "string" && hh.indexOf("%") > 0)
					...
					if (_._ba === "slide") {
						...
						if (_._isstatic && opt.carousel && opt.carousel.horizontal_align && opt.sliderType === "carousel") {
					...
					if (_nc.hasClass("tp-videolayer") || _nc.hasClass("tp-audiolayer") || _nc.find('iframe').length > 0 || _nc.find(tag).length > 0) {
						...
						if (_._ingroup) {
							if (_.videowidth !== null && _.videowidth && !jQuery.isNumeric(_.videowidth) && _.videowidth.indexOf("%") > 0)
					...
					if (_._nctype === "column") {
						var tempy = _nc[0]._gsTransform ? _nc[0]._gsTransform.y : 0,
					...
					if (_._ingroup && _._nctype !== "row") {
						if (_._groupw && !jQuery.isNumeric(_._groupw) && _._groupw.indexOf("%") > 0)
							punchgs.TweenLite.set([_._lw, _._pw, _._mw], { minWidth: _._groupw });

						if (_._grouph && !jQuery.isNumeric(_._grouph) && _._grouph.indexOf("%") > 0)
			...
			***************************************/
			buildFullTimeLine: function (obj) {
				...
				if (_.current_timeline && obj.regenerate !== true) {
				...
				// HOVER ANIMATION
				if (_.hoverframeindex !== -1 && _.hoverframeindex) {
						...
						_nc.hover(function (e) {
							...
							function (e) {
								...
								if (animended) {
									...
									if (_.hovertimelines.pwhoveranim) _.hovertimelines.pwhoveranim = punchgs.TweenLite.to(_._pw, frame.speed / 1000, { overwrite: "auto", zIndex: _.basiczindex });
				...
				for (var frame_index = 0; frame_index < _.frames.length; frame_index++) {
					...
					if (frame_index !== _.hoverframeindex) {
						...
						var $start = _.frames[frame_index].delay,
							...
							mdelay = $start ? jQuery.inArray($start, ["slideenter", "bytrigger", "wait"]) >= 0 ? $start : parseInt($start, 0) / 1000 : "wait";


						// ADD STARTLABEL FOR STATIC LAYERS 
						if (_nc_tl_obj.firstslide && frame_name === "frame_0") {
						...
						if (_nc_tl_obj.lastslide && frame_name === "frame_999") {
			...
			createFrameOnTimeline: function (obj) {
				...
				if (_.splittext && frame_index === 0) {
					if (_.mySplitText) _.mySplitText.revert();
				...
				if (_.mySplitText && $split && $split.match(/chars|words|lines/g)) {
				...
				var $to = frame_index !== _.outframeindex ? getAnimDatas(newAnimObject(), _.frames[frame_index].to, undefined, $splitnow, animobject.length - 1) : _.frames[frame_index].to && _.frames[frame_index].to.match(/auto:auto/g) === null ? getAnimDatas(newEndAnimObject(), _.frames[frame_index].to, opt.sdir == 1, $splitnow, (animobject.length - 1)) : getAnimDatas(newEndAnimObject(), _.frames[_.inframeindex].from, opt.sdir == 0, $splitnow, (animobject.length - 1)),
					$from = _.frames[frame_index].from ? getAnimDatas($to, _.frames[_.inframeindex].from, opt.sdir == 1, $splitnow, animobject.length - 1) : undefined, 		// ANIMATE FROM THE VERY FIRST SETTING, OR FROM PREVIOUS SETTING 				
				...
				if (frame_index === 0 && !obj.fromcurrentstate) {
					...
					if (animobject != _nc) {
						...
						$to = newAnimObject();
						$to.ease = old.ease;
						if (old.filter) $to.anim.filter = old.filter;
						if (old["-webkit-filter"]) $to.anim["-webkit-filter"] = old["-webkit-filter"];
					}
				...
				if ((frame_index === 0 && !obj.fromcurrentstate && _._bmask && $sfx && $sfx.indexOf("block") >= 0) ||
					(frame_index === _.outframeindex && !obj.fromcurrentstate && _._bmask && $sfx && $sfx.indexOf("block") >= 0)) {
				...
				if ($mask_to && $mask_to !== false && (frame_index !== 0 || !obj.ignorefirstframe)) {
			...
			playAnimationFrame: function (obj) {
				...
				if (_.triggeredtimeline) _.triggeredtimeline.pause();
	...
	**********************************************************************************************/
	...
	var tweenOnStart = function (frame_index, ncobj, pw, _, tl, toanim, _nc, ust, opt) {
		...
		if (ncobj.dchildren !== "none" && ncobj.dchildren && ncobj.dchildren.length > 0) {
			if (frame_index === 0)
	...
	var tweenOnUpdate = function (label, id, pw, _, tl, frame_index, toanim, ust, _nc, opt) {
		...
		if (_.hoveranim && _._gsTransformTo === false) {
	...
	var setResponsiveCSSValues = function (obj, opt) {
		...
		if (obj)
			jQuery.each(obj, function (key, val) {
				...
				newobj[key] = res_a ? res_a : obj[key];
			});
		return newobj;
	}
	...
	var calcCaptionResponsive = function (nc, opt, level, responsive) {
		...
		if (!nc.hasClass("tp-splitted")) {
			...
			var hashover = nc.data('transform_hover') || nc.data('style_hover');
