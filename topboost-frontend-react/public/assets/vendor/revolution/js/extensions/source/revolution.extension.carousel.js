...
(function ($) {
		...
		organiseCarousel: function (opt, direction, setmaind, unli) {
			...
			if (_.slides)
				jQuery.each(_.slides, function (i, slide) {
					...
					// ROTATION FUNCTIONS		
					if (_.maxRotation && Math.abs(_.maxRotation) != 0) {
					...
					tr.scale ? d < 0 ? tr.x - sx : tr.x + sx : tr.x;
	...
	***************************************************/
	...
	var defineCarouselElements = function (opt) {
		...
		if (_.border_radius && parseInt(_.border_radius, 0) > 0) {
	...
	var getActiveCarouselOffset = function (opt) {
		...
		if (_.positionanim) _.positionanim.kill();
