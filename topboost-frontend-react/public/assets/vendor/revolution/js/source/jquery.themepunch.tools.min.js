...
(function (factory) {
	...
	*/
	function init(options) {
		...
		if (options && (options.allowPageScroll === undefined && (options.swipe || options.swipeStatus))) {
			options.allowPageScroll = NONE;
		}

		//Check for deprecated options
		//Ensure that any old click handlers are assigned to the new tap, unless we have a tap
		if (options.click && options.tap === undefined) {
	...
	function TouchSwipe(element, options) {
		...
		*/
		function updateFingerData(evt) {
			var id = evt.identifier ? evt.identifier : 0;
			...
			_cssDif = function (t, s1, s2, vars, forceLookup) {
				...
				for (p in s2) {
					if (p !== "cssText") if (p !== "length") if (isNaN(p)) if (s1[p] !== (val = s2[p]) || (forceLookup && forceLookup[p])) if (p.indexOf("Origin") === -1) if (typeof (val) === "number" || typeof (val) === "string") {
						...
						if (style[p]) { //for className tweens, we must remember which properties already existed inline - the ones that didn't should be removed when the tween isn't in progress because they were only introduced to facilitate the transition between classes.
