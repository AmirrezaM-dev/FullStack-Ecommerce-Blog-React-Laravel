...
; (function (jQuery, undefined) {
	...
	jQuery.fn.extend({
		...
		revstart: function () {
			//return this.each(function() {
			var c = jQuery(this);
			if (c != undefined && c.length > 0 && jQuery('body').find('#' + c.attr('id')).length > 0 && c[0].opt) {
...
var runSlider = function (container, opt) {
	...
	jQuery.each(opt.allli, function (index, li) {
		...
		obj.src = li.data('thumb') ? li.data('thumb') : img.data('lazyload') ? img.data('lazyload') : img.attr('src');
		if (li.data('title')) obj.params.push({ from: RegExp("\\{\\{title\\}\\}", "g"), to: li.data("title") })
		if (li.data('description')) obj.params.push({ from: RegExp("\\{\\{description\\}\\}", "g"), to: li.data("description") })
		for (var i = 1; i <= 10; i++) {
			if (li.data("param" + i))
	...
	if (!container.hasClass("revslider-initialised")) {
		...
		container.find('.tp-caption, .rs-background-video-layer').each(function (i) {
				var _nc = jQuery(this),
					...
					ap = _.autoplay,
					htmlvideo = _.videomp4 || _.videowebm || _.videoogv,
				...
				if (_nc.hasClass("tp-caption")) {
					...
					if (_ndata.wrapper_class) preclas = preclas + " " + _ndata.wrapper_class;
					if (_ndata.wrapper_id) preid = 'id="' + _ndata.wrapper_id + '"';
				...
				if (as) _R.checkActions(_nc, opt, as);
...
var cArray = function (b, l) {
			...
			var checkHoverDependencies = function (_nc, opt) {
				var _ = _nc.data(),
					senter = _.start === "sliderenter" || (_.frames && _.frames[0] != undefined && _.frames[0].delay === "sliderenter") ? true : false;
			...
			var containerResized = function (c, opt) {
				...
				if (!_R.resizeThumbsTabs || _R.resizeThumbsTabs(opt) === true) {
					...
					if (nextsh.data('kenburns') == "on")
						_R.startKenBurn(nextsh, opt, (nextsh.data('kbtl') ? nextsh.data('kbtl').progress() : 0));

					if (actsh.data('kenburns') == "on")
						_R.startKenBurn(actsh, opt, (actsh.data('kbtl') ? actsh.data('kbtl').progress() : 0));
			...
			var prepareSlides = function (container, opt) {
				...
				container.find('.tp-caption').each(function () {
					var c = jQuery(this);
					if (c.data('transition')) c.addClass(c.data('transition'));
				});
				...
				opt.allli.find('>img').each(function (j) {
					...
					if (bgcolor && bgcolor.indexOf('gradient') >= 0)
			...
			var loadImages = function (container, opt, prio, staticlayer) {
				...
				container.find('img,.defaultimg, .tp-svg-layer').each(function () {
					var element = jQuery(this),
						src = element.data('lazyload') && element.data('lazyload') !== "undefined" ? element.data('lazyload') : element.data('svg_src') != undefined ? element.data('svg_src') : element.attr('src'),
		...
		if (element.data('loaded') === undefined && loadobj && loadobj.progress && loadobj.progress.match(/loaded/g)) {
...
var swapSlide = function (container) {
	...
	opt.cd = 0;
	if (opt.spinner === "off")
		if (opt.loader) opt.loader.css({ display: "none" });
		else
			opt.loadertimer = setTimeout(function () { if (opt.loader) opt.loader.css({ display: "block" }); }, 50);
...
var swapSlideProgress = function (defimg, container) {
	...
	nexttrans = 0;
	var transtext = nextli.data('transition') ? nextli.data('transition').split(",") : "fade",
...
var letItFree = function (container, nextsh, actsh, nextli, actli, mtl) {
	...
	if (opt.startWithSlide && opt.startWithSlide !== "done" && opt.sliderType === "carousel") {
