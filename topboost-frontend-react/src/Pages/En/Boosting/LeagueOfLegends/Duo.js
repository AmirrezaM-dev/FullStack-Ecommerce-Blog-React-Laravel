...
const Duo = () => {
	...
	const [duoCodeTypeOfQueue, setDuoCodeTypeOfQueue] = useState()
	const [duoCodeTypeOfQueues, setDuoCodeTypeOfQueues] = useState()
	const duoCodeTypeOfQueueRef = useRef()
	const [duoCodeTypeOfQueueForce, setDuoCodeTypeOfQueueForce] = useState({
		menuIsOpen: false,
		forced: false,
	})
	...
	useEffect(() => {
		if (duoCodeFirstLoaded) {
			...
			if (duoCodeTypeOfQueue) {
				add_price =
					add_price + (price / 100) * duoCodeTypeOfQueue.percentage
			}
	...
	useEffect(() => {
		if (duoCodeRequestToPay) {
			...
			if (duoCodeTypeOfQueue && duoCodeTypeOfQueue.length !== 0) {
				setDuoCodeTypeOfQueueForce({ menuIsOpen: false, forced: false })
			} else {
				setDuoCodeTypeOfQueueForce({
					menuIsOpen: !error ? true : false,
					forced: true,
				})
				error = 1
	...
	const duoCodeCheckPurchase = () => {
		...
		if (
			duoCodeTypeOfQueue === undefined ||
			duoCodeTypeOfQueue.length === 0
		) {
			if (!error) {
				duoCodeTypeOfQueueRef.current.select.controlRef.scrollIntoView({
					behavior: "smooth",
					block: "center",
					inline: "nearest",
				})
	...
	const duoCodePurchase = () => {
		if (duoCodeCheckPurchase() && !paying) {
			...
			api()
				.post("/LeagueOfLegends/duoCode/makePurchase", {
					code: duoCode,
					...
					typeOfQueue: duoCodeTypeOfQueue.value,
			...
			<div className="col-md-12">
				<div className="row vertical-gap">
					...
					<div className="col-md-3">
						{duoCodeFirstLoaded ? (
							<Select
								ref={duoCodeTypeOfQueueRef}
								menuIsOpen={duoCodeTypeOfQueueForce.menuIsOpen}
								onMenuOpen={() =>
									setDuoCodeTypeOfQueueForce(
										(prevDuoCodeTypeOfQueueForce) => {
											return {
												...prevDuoCodeTypeOfQueueForce,
												menuIsOpen:
													!prevDuoCodeTypeOfQueueForce.menuIsOpen,
											}
										}
									)
								}
								...
								className={
									duoCodeTypeOfQueueForce.forced
										? "custom-forced"
										: ""
								}
								placeholder="Type of queue"
								onChange={(e) => {
									setDuoCodeTypeOfQueue(
										duoCodeTypeOfQueues[
											objSearch(
												duoCodeTypeOfQueues,
												"value",
								...
								options={duoCodeTypeOfQueues}
								value={duoCodeTypeOfQueue}
								isSearchable={false}
								styles={customStyles}
								theme={(theme) => ({
									...theme,
