...
const PlacementMatches = () => {
	...
	const [placementMatchCodeAgents, setPlacementMatchCodeAgents] = useState()
	const [placementMatchCodeAgent, setPlacementMatchCodeAgent] = useState([])
	const [placementMatchCodeOnlyOneAgent, setPlacementMatchCodeOnlyOneAgent] =
		useState(false)
	const [
	...
	const placementMatchCodeCancelSpecific = (item) => {
		...
		setPlacementMatchCodeAgent(null)
		document.body.style.overflow = "auto"
		document.getElementsByClassName(
			"specific_" + placementMatchCode
		)[0].checked = false
	}
	const placementMatchCodeApplySpecific = () => {
		let error = 0
		if (
			(!placementMatchCodeAgent ||
				placementMatchCodeAgent === null ||
				placementMatchCodeAgent.length <
					(placementMatchCodeOnlyOneAgent ? 1 : 3)) &&
			error === 0
		) {
			Toast.fire({
				icon: "error",
		...
		if (placementMatchCodeFirstLoaded) {
			...
			if (
				...
				add_price =
					add_price +
					(price / 100) *
						(!placementMatchCodeOnlyOneAgent
							? placementMatchCodeFirstData.vip.filter(
									(item) =>
	...
	useEffect(() => {
		if (!placementMatchCodeFirstLoaded) {
			...
			setPlacementMatchCodeAgents(data.agents)
	...
	const placementMatchCodePurchase = () => {
		if (placementMatchCodeCheckPurchase() && !paying) {
			...
			api()
				.post("/Valorant/placementMatchCode/makePurchase", {
					...
					typeOfCustom: placementMatchCodeTypeOfDuo
						...
						: null,
					agents: placementMatchCodeAgent
						? placementMatchCodeAgent
						: null,
					onlyOneAgent: placementMatchCodeOnlyOneAgent
						? placementMatchCodeOnlyOneAgent
						: false,
							...
							<div className="row vertical-gap pl-10 pr-10">
								<div className="col-12">
									<Select
										...
										onChange={(e) => {
											setPlacementMatchCodeAgent(e)
										}}
										options={placementMatchCodeAgents}
										value={placementMatchCodeAgent}
										isMulti
								...
								<div className="col-12 pt-10 pb-10">
									<div className="custom-control custom-checkbox">
										<input
											onChange={(e) =>
												setPlacementMatchCodeOnlyOneAgent(
													e.target.checked
												)
											}
											type="checkbox"
