...
class TftPlacementMatchesController extends Controller
{
	public function makePurchase(Request $request)
	{
		...
		$cart = [];
		$error = 0;
		if ($request->input("currentRank") == null && $request->input("currentRank") !== 0) {
			$error = "currentRank";
		}
		if ($request->input("currentDivision") == null && $request->input("currentDivision") !== 0) {
		...
		if (!$error) {
			...
			$price = $workingCurrentDivision["placement_matches_price"] * $request->input("numberOfGames");

			$cart[] = [$request->input("numberOfGames") . " game win in placement matches for " . $workingCurrentRank["label"], (round($price, 2, PHP_ROUND_HALF_UP) * pow(10, 2)) / pow(10, 2), true];
			...
			$cart[] = ["Order", $request->input("numberOfGames") . " game win in placement matches for " . $workingCurrentRank["label"], false];
		...
		$newCheckout = Checkouts::create([
			...
			'code' => $randomString,
			'title' => "You've purchased teamfight tactics placement matches boost:",
