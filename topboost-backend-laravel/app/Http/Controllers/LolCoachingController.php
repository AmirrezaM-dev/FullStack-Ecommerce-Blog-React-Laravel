...
class LolCoachingController extends Controller
{
	public function makePurchase(Request $request)
	{
		...
		if (!$error) {
			...
			$cart[] = ["Type of coaching", LolTOC::find($request->input("typeOfCustom"))->name, false];


			$lines = "";
			if (count($request->input("line"))) {
				foreach ($request->input("line") as &$value) {
					$gotData = LolRoles::find($value["value"]);
					if ($gotData->count()) {
						$lines .= (strlen($lines) ? ", " . $gotData->name : $gotData->name);
					}
				}
			}
			$cart[] = ["Line", $lines, false];
		...
		$newCheckout = Checkouts::create([
			...
			'code' => $randomString,
			'title' => "You've purchased league of legends coaching:",
